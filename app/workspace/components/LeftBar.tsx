...
const LeftSidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Left Sidebar */}
      <div
        className={`lg:relative fixed lg:left-0 lg:top-0 top-0 h-full w-64 shadow-xl flex flex-col p-6 transition-all duration-300 ease-in-out z-20 lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
  ...
  {/* Notebooks List */}
  <div className="flex flex-col overflow-y-auto" style={{ maxHeight: 'calc(100vh - 150px)' }}>
      ...
      {notebooks.map((notebook, notebookIndex) => (
        ...
        >
          ...
          {dropdownIndex === notebookIndex && (
            <div
              ref={dropdownRef}
              className="border absolute top-full right-0 w-40 bg-white dark:bg-black shadow-md rounded p-2 z-30"
            >
        ...
        {/* Bottom Section */}
        <div className="flex text-sm items-end justify-between absolute bottom-0 left-0 w-full p-3 pl-6   bg-white dark:bg-black text-black dark:text-white transition-all duration-300">
